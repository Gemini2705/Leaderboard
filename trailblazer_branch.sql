-- Current Month Trailblazer: Best monthly performance in current FY till current month
current_month_trailblazer AS (
    SELECT 
        dc.current_month_name,
        dc.financial_year,
        branch_best.state_name,
        branch_best.branch_name,
        branch_best.branch_code,
        branch_best.best_monthly_amount,
        branch_best.best_month_period,
        RANK() OVER (ORDER BY branch_best.best_monthly_amount DESC) as current_month_rank
    FROM (
        SELECT 
            md.state_name,
            md.branch_name,
        ...
        GROUP BY md.state_name, md.branch_name, md.branch_code
    ) branch_best
    CROSS JOIN date_calculations dc
),
...
previous_month_trailblazer AS (
    SELECT 
        ...
        branch_best.best_monthly_amount,
        branch_best.best_month_period,
        RANK() OVER (ORDER BY branch_best.best_monthly_amount DESC) as previous_month_rank
    FROM (
        SELECT 
            md.state_name,
            md.branch_name,
...
current_quarter_trailblazer AS (
    SELECT 
        ...
        branch_best.branch_name,
        branch_best.branch_code,
        branch_best.best_quarterly_amount,
        branch_best.best_quarter_period,
        RANK() OVER (ORDER BY branch_best.best_quarterly_amount DESC) as current_quarter_rank
    FROM (
        SELECT 
            qd.state_name,
            qd.branch_name,
        ...
        GROUP BY qd.state_name, qd.branch_name, qd.branch_code
    ) branch_best
    CROSS JOIN date_calculations dc
),
...
-- Final trailblazer ranking combining all categories
SELECT 
