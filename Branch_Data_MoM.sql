...
disbursal_base AS (
       SELECT 
        ...
        ai."Sourcing RM Name" as sourcing_rm_name,
        ai."Sourcing RM Code" as sourcing_rm_code,
...
-- Monthly aggregations for each branch with TAT calculation
monthly_data AS (
    SELECT 
        state_name,
        branch_name,
    ...
    GROUP BY state_name, branch_name, branch_code, disb_month, disb_month_num, disb_year, disb_financial_year, disb_quarter
),
...
quarterly_data AS (
    ...
    FROM monthly_data
    GROUP BY state_name, branch_name, branch_code, disb_financial_year, disb_quarter
...
yearly_data AS (
    ...
    FROM monthly_data
    GROUP BY state_name, branch_name, branch_code, disb_financial_year
    HAVING SUM(monthly_disbursal_amount) > 0
),
...
login_data AS (
    ...
    GROUP BY state_name, branch_name, branch_code, app_month, app_month_num, app_year, app_financial_year, app_quarter
),
...
current_quarter_login AS (
    ...
    GROUP BY state_name, branch_name, branch_code, app_financial_year, app_quarter
),
...
current_fy_login AS (
    ...
    GROUP BY state_name, branch_name, branch_code, app_financial_year
),

all_branch_months AS (
    SELECT DISTINCT 
        all_branches.state_name, all_branches.branch_name, all_branches.branch_code, 
        all_months.month_name
    FROM (
        SELECT state_name, branch_name, branch_code FROM disbursal_data
        UNION
        SELECT state_name, branch_name, branch_code FROM login_data
    ) all_branches
    CROSS JOIN (
...
combined_data AS (
    ...
    FROM all_branch_months abm
    LEFT JOIN login_data ld ON (
        abm.state_name = ld.state_name 
        AND abm.branch_name = ld.branch_name 
        AND abm.branch_code = ld.branch_code
        AND abm.month_name = ld.app_month
    )
    LEFT JOIN disbursal_data dd ON (
        abm.state_name = dd.state_name 
        AND abm.branch_name = dd.branch_name 
        AND abm.branch_code = dd.branch_code
        AND abm.month_name = dd.disb_month
    )
)
...
FROM combined_data
GROUP BY state_name, branch_name, branch_code
